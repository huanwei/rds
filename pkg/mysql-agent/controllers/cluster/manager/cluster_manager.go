...
import (
	...
	"github.com/huanwei/rds/pkg/mysql-agent/cluster"
...
func (m *ClusterManager) Sync(ctx context.Context) bool {
	...
	if err != nil {
		...
		if m.Instance.Ordinal == 0 {
			...
			glog.V(2).Infof("Database successfully bootstraped.")
		} else {
	...
	case innodb.InstanceStatusMissing:
		...
		if online {
			glog.V(4).Info("MySQL instance rejoin success")
		} else {
			glog.V(4).Info("MySQL instance rejoin fail")
		}

	case innodb.InstanceStatusNotFound:
		glog.V(4).Info("MySQL instance status Not Found")
		primaryAddr, err := clusterStatus.GetPrimaryAddr()
		if err != nil {
			glog.Errorf("%v", err)
			return false
		...
		if online {
			glog.V(4).Info("MySQL instance add success")

		} else {
			glog.V(4).Info("MySQL instance add fail")
		}

	case innodb.InstanceStatusUnreachable:
		glog.V(4).Info("MySQL instance unreachable")

...
const (
	...
	mysqlAgent  = "huanwei/mysql-agent"
)
...
func (s *MySQLOperatorOpts) AddFlags(fs *pflag.FlagSet) *pflag.FlagSet {
	...
	fs.StringVar(&s.Images.MySQLAgentImage, "mysql-agent-image", s.Images.MySQLAgentImage, "The name of the target 'mysql-agent' image. Defaults to: huanwei/mysql-agent.")
